...
class DataFrame {
    ...
    /**
     * The promisified version of {@link DataFrame#columns}
     * @since 1.3.0
     */
    columnsPromised()  /*: (err: any, res: Array[String]) =>  any */ {
        //return this.jvm_obj.columns();
        return new Promise((resolve, reject) => {
                let jvm_cb =  (err, columns) => {
                    if (err) reject(err);
                    resolve(columns);
                };
            this.jvm_obj.columnsAsync(jvm_cb);
        });
    }
    ...
    /**
     * The promisified version of {@link DataFrame#head}
     *
     * @param {Number} [n=1] Number of rows to return.
     *
     * @since 1.3.0
     */
    headPromised(n=1 /*: Int */) /*: Array[Object] */ {
        return new Promise((resolve, reject) => {
                let jvm_cb =  (err, jvm_rows) => {
                    if (err) reject(err);
                    resolve(jvm_rows.map(row => new Row(row).values()));
                };

                this.jvm_obj.headAsync(n, jvm_cb);
            });
    }
    ...
    /**
     * The promisified version of {@link DataFrame#collect}
     *
     * @since 1.3.0
     */
    collectPromised() /*: Array[Object]*/ {
        return new Promise((resolve, reject) => {
                let jvm_cb =  (err, jvm_rows) => {
                    if (err) reject(err);
                    resolve(jvm_rows.map(row => new Row(row).values()));
    ...
    /**
     * The promisified version of {@link DataFrame#count}
     * @since 1.3.0
     */
    countPromised() /*: Number */ {
        return new Promise((resolve, reject) => {
                let jvm_cb =  (err, res) => {
                    if (err) reject(err);
                    resolve(res.valueOf());
